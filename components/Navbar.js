...
function Navbar() {
  ...
  return (
    <div className="bg-transparent text-white shadow-lg fixed top-0 left-0 right-0 z-50">
        ...
        >
          {Meuns.map((menu) => {
            return (
              <li
                key={menu.name}
                className={`md:ml-4 my-4 md:my-0 text-white ${asPath === `${menu.path}` && "text-white md:border-b-2 mb-1 md:border-[#263C41] "}`}
              >
